function [results] = XYZmatch(subFrame_results, epsilon, minpts, maxMatchErr)
    % 原始
    figure();
    subplot(2,2,[1,2]);
    ...
    cnt_subFrame0obj = 0;
    xyz = subFrame_results(1).xyz;
    idx = dbscan(xyz(:, 1:3),epsilon,minpts);
    subplot(2,2,3);
    classIds = unique(idx);
    legend_list = {};
    ...
    for sub0ObjId = 1:cnt_subFrame0obj
        ...
        for sub1ObjId = 1:cnt_subFrame1obj
            errorDis_list(sub1ObjId) = sqrt((subFrame0ObjInfo(sub0ObjId).objX - subFrame1ObjInfo(sub1ObjId).objX).^2 + ...
                ...
                (subFrame0ObjInfo(sub0ObjId).objZ - subFrame1ObjInfo(sub1ObjId).objZ).^2);
        end
